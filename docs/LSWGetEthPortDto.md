...
**isSupportAutoNegotiation** | **Boolean** | 端口是否支持关闭自协商（只读），根据款型确定是否支持。 |  [optional]
**isSupportPoe** | **Boolean** | 端口是否支持POE使能（只读）。 |  [optional]
**media** | **String** | 端口模式（只读）。fiber：光口模式；copper：电口模式。 |  [optional]
**isUpStream** | **Boolean** | 端口是否为上行口（只读）。 |  [optional]
